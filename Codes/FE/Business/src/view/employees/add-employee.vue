            ...
            <div class="form" style="width: 100%; padding: 20px 30px; border: 1px solid #e4e4e4;">
                ...
                <div >
                    ...
                    <div style="display: inline-block; vertical-align: top;">
                        <slideBtn :slideBtnOpt="employee" active-text="启用" inactive-text="禁用" 
                        :prop-value-map="{0:0,1:1}" prop-key="state"
                        @slide-event="employeeState"/>
                    </div>
                </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getEmployeeInfo(opt, cb) {
      ...
      api.request('getEmployeeInfo', {id: opt })
    ...
    formValidate() {
      //表单验证
      let objArr = [$("#name"), $("#phone")];
      let _name = window.formValidation({
        obj: objArr[0],
        valueLength: 10,
        minLength: 2,
        empty: "请输入人员姓名",
        lengthError: "人员姓名为2~10个中文字符",
        reg: /^[\u4e00-\u9fa5]+$/,
        error: "人员姓名为2~10个中文字符"
      });
    ...
    submit() {
      ...
      api.request("createEmployee", _this.employee, result => {
        if (result.status == 0) {
