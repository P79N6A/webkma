...
angular.module('managementApp')
  .controller('MaterialShapCtrl', ["$scope", "materialListService", "messenger","userListService","$location","$timeout",
    function ($scope, materialListService, messenger,userListService,$location,$timeout) {
        ...
        materialListService.getMaterialClassList(opt, function (result) {
          if (result.code == 0) {
            $.each(result.data.list,function(index,item){
                if(item.key == 'controls_shape'){
                    $scope.searchOptions.materialCategoryId = item.material_id;
                }
            });
            $scope.getImgMaterialList();//获取形状素材列表
            $scope.getMaterialTagList();//获取形状标签
          }else {
      ...
      $scope.getImgMaterialList = function (callback) {
        ...
        materialListService.getMaterialList(options, function (json) {
          if (json.code === 0) {
            ...
            $scope.$broadcast("MaterialShapflipupdateTotalcount",$scope.pagination.totalCount);
            ...
            $.each(json.data.list,function(index,item){
                switch (item.materialType){
                    case 1:
                      ...
                      break;
                    case 2:
                      item['materialTypeName'] = 'svg';
                      break;
                    case 3:
                      item['materialTypeName'] = '组合素材';
                      break;
                }
                item.active = false;
            });
            ...
            messenger.error('获取形状素材列表失败:' + json.desc);
          }
          !!callback && callback();
        });
      };
      ...
      // 分页切换
      $scope.$on("MaterialShapflip",function(e,pageIndex,callback){//所在地选取
      ...
      $scope.getMaterialTagList = function(){
          var _data = {
              ...
              tagType: '8',//标签类别，1:模板 ,2:H5,3:短信,4:邮件，5客户 6文本 7图片 8形态 9背景  如果为null 或者 为 0则查询全部标签
