...
angular.module('managementApp')
    .controller('TemplateListCtrl', ['$scope', 'messenger', '$location', 'templateService', 'businessService',
        function ($scope, messenger, $location, templateService, businessService) {
            this.awesomeThings = [
                'HTML5 Boilerplate',
                'AngularJS',
                'Karma'
            ...
            if (!!$scope.designerId)
                $scope.$root.$emit('reloadLeftMenu', 'template_management', 0);
            ...
            $scope.getTempList = function (callback) {
                var obj = {
                    ...
                    search: $scope.searchKeyword.module,
                    designerId: $scope.designerId
                }
                templateService.getTempList(obj, function (res) {
                    if (res.status == 0) {
            ...
            $scope.getBusinessList = function (callback) {
                var obj = {
                    ...
                    manuscriptId: $scope.business.manuscriptId,//稿件/模板id 选填
            ...
            $scope.action = function (type, data) {
                switch (type) {
                    ...
                    case "handout"://点击分发
                        ...
                        $scope.business.manuscriptId = data.id;
                        $scope.business.type = data.distribution;
                    ...
                    case "handoutBusiness"://分发弹框点击确认
                        templateService.handoutBusiness({
                            id: $scope.business.manuscriptId,
            ...
            $scope.delTemplate = function () {
                templateService.delTemplate({ "id": $scope.prompt.delId }, function (res) {
                    if (res.status == 0) {
