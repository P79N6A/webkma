<template>
    ...
    <fold title="设计师管理">
      <el-row class="operation">
        <el-button type="primary" @click="toManagementPage('')">新增设计师</el-button>
      ...
      <el-table
              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    <span class="btn-plain" @click="toManagementPage(scope.row)">编辑</span>
...
export default {
  name:'designer-list',
  components: { search, pagination, slideBtn, fold},
	data() {
		return {
      bussinessList: [], //商家列表数据
      businesskeyword: '', //关键词
      searchOpt: {
        placeholder: '请输入账号、设计师姓名'
      },
      paginationOpt: {
        pageIndex: 1,
        pageCount: '',
  ...
  methods: {
    ...
    toManagementPage(item) {
      ...
      this.$router.push({
        path: `/designer-management${param}`
      });
    },
    //设置状态
    setSingleState(data) {
    ...
    deleteDesigner(item) {
      let _this = this;
      this.messenger.confirm('你确定要删除该设计师吗？',function(tag){
          if(tag){
              api.request('deletebusiness', { businessId: item.businessId }, json => {
                  if (json.status == 0) {
                      _this.getList();
                      _this.messenger.success("操作成功");
                  } else {
                      _this.messenger.error("操作失败:" + json.message);
                  }
              });
          }
      })
