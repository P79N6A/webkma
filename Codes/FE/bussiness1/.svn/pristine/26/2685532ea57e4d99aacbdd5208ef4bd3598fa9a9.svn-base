    ...
    <template v-if="mode == 'transfer'">
      ...
      <div class="transfer-left">
        <h3 class="transfer-title">
          ...
          >全选</el-checkbox>
          <label class="label-title">{{fromTitle}}</label>
        </h3>
        <!-- 内容区 -->
        <div class="transfer-main">
          ...
          <el-tree
            ...
            @node-expand="eventFromExpand"
            @node-collapse="eventFromCollapse"
          ></el-tree>
        </div>
      </div>
      <!-- 穿梭区 按钮框 -->
      ...
      <div class="transfer-right">
        <h3 class="transfer-title">
          ...
          >全选</el-checkbox>
          <label class="label-title">{{toTitle}}</label>
        </h3>
        <!-- 内容区 -->
        <div class="transfer-main">
          <!-- <slot name='to'></slot> -->
...
export default {
  ...
  methods: {
    ...
    removeToSource() {
      ...
      // 目标数据节点展开
      if (this.transferOpenNode) {
        // this.from_expanded_keys = Array.from(
        //   new Set((this.from_expanded_keys || []).concat(keys))
        // );
      }
    ...
    eventFromExpand(data) {
      if ((data.children || []).length == 0) return;
      this.from_expanded_keys = (this.from_expanded_keys || []).concat(data.id);
    },
    eventFromCollapse(data) {
      this.from_expanded_keys = (this.from_expanded_keys || []).filter(item => {
        return item !== data.id;
      });
    }
  },
...
.transfer .el-tree-node__content > .el-checkbox {
  margin: 0 8px 0 0;
}
.transfer .el-tree-node__label {
  font-size: 12px;
}
.transfer .el-tree__empty-block {
  min-height: 30px;
}
.transfer .el-tree__empty-block .el-tree__empty-text {
  font-size: 12px;
  color: #909399;
}
</style>
