...
export default {
    ...
    methods: {
        ...
        bindUploadAudio() {
            let self = this;
            setTimeout(() => {
                // 兼容部分浏览器在打开windows选择窗口时，选择文件用双击方式后触发编辑区mouseup事件，导致模块切换或取消选中状态。
                $("#uploadAudio").off("click").on("click", function() { 
                    Kdo.events.off("mouse");
                    $("body").one("mouseup", function() { setTimeout(function(){ Kdo.events.on("mouse");}); });
                });
