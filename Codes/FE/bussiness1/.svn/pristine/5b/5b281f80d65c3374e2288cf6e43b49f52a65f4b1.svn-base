  ...
  <div class="pull-left tpl-column" :class="{'on': data && data.enable == 0}">
    ...
    <div v-if="options.category == 'create'">
      <div style="cursor:pointer;" @click="btnCreate()">
        <div class="tpl-panel">
          <div style="padding-top:89px; padding-bottom:30px;">
            <img style="width: 50px; height: 50px;" src="../assets/images/add.png">
          </div>
          <div class="font-12 black-deep" v-if="options.manuscriptType=='1'">新建H5活动</div>
          <div class="font-12 black-deep" v-if="options.manuscriptType=='2'">新建海报</div>
        </div>
    ...
    <div v-if="options.category == 'detail'">
      <div class="tpl-panel">
        <div class="lock-status" v-if="data.isUnlock === 0">
          <div class="status-bg">
            <div class="img-lock"></div>
          </div>
        </div>
        <div class="image-box" :style="{backgroundImage: data.cover ? `url('${data.cover}')` : false, backgroundPosition: data.cover ? 'top' : false }"></div>
        <div class="tpl-cover" v-if="data && data.enable != 0">
          <div style="margin-top:20px; margin-bottom: 12px;">
            <img v-if="data.type===3&&data.goodsNumber==0" class="weapp-qrcode" :src="data.codeUrl || '/static/images/logo-white.png'">
            ...
            <table style="margin: 0 auto;">
              <tr>
                <td class="btn-action" :style="{'margin-right': (index+1 != options.actions.length) ? '20px': false}"
                  @click="btnAction(actionEnums[key].type, data)" v-for="(key, index) in options.actions" :key="key">
      ...
      <div class="title-desc" :title="data.name">
        <p style="width:120px;overflow: hidden;text-overflow: -o-ellipsis-lastline;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;" class="font-12 black-deep">{{data.name}}</p>
        <p class="attrs" v-if="data.useTotal != null" style="position:absolute;bottom:10px;">
          <i class="el-icon-view" style="font-size: 14px; margin-right: 4px;"></i>
          <span>{{data.useTotal}}</span>
        </p>
        <p class="attrs" v-else style="position:absolute;bottom:10px;">
    ...
    <div v-if="options.category == 'qrcode'" class="qrcode-item" :class="[data.active ? 'active': '']" @click="selectH5(data)">
      ...
      <div class="title-desc" :title="data.name">
        <p style="width:180px; height:34px; line-height:34px; overflow:hidden; white-space:nowrap; text-overflow:ellipsis; "
          class="font-14 black-deep">{{data.name}}</p>
...
export default {
  ...
  methods: {
    btnAction(action, item) {
      ...
      switch (action) {
        ...
        case "use":
          ...
          if (item.isUnlock === 0) {
            if (!self.options.canUnlock) return Kdo.utils.messenger.alert("<div style='text-align:center;'>您可用的活动模板已达到上限，请联系推小宝升级！</div>");
                  ...
                  api.request(
                    ...
                    function (result) {
                      if (result.status !== 0) {
                        ...
                        cb(true);
                        item.isUnlock = 1;
                        eventBus.$emit('manuscript.list.addTplUse');
                        setTimeout(() => {
    ...
    btnCreate(templateId, callback, manuscriptType) {
      var self = this;
      Kdo.utils.asyncOpenWindow(childWindow => {
        api.request(
          "createManuscript",
          {
            ...
            manuscriptType: !!manuscriptType ? manuscriptType : this.options.manuscriptType
          },
          function (result) {
            if (result.status !== 0) {
              Kdo.utils.messenger.error(result.message);
              ...
              let routeData = self.$router.resolve({
                name: "design",
                query: { id: result.data.id, goodsIds: self.options.goodsIds, kid: self.options.kid, manuscriptType: !!manuscriptType ? manuscriptType : self.options.manuscriptType }
              });
    ...
    createQrcode() {
      if (!!this.data && (this.data.type !== 3 || this.data.goodsNumber > 0)) {
        let url = this.data.url;
        url = /^(http|https):/i.test(url)
          ? url
          : location.protocol + "//" + url;
        let qrcode = new QRCode(this.$el.querySelector(".normal-qrcode"), {
          text: url + ((this.data.goodsNumber > 0 && this.data.type == 3) ? "" : "?session_id=" + localStorage.sessionId + "&preview=true"),
          width: 90,
          height: 90,
          colorDark: "#000000",
          colorLight: "#ffffff",
...
.tpl-table .tpl-column > div{
  width: 150px;
  height: 280px;
}
.tpl-table .tpl-column > div.qrcode-item {
  width: 200px;
  height: 265px;
}
...
.tpl-table .tpl-column > div.qrcode-item .tpl-panel{
  height: 263px;
}
...
.tpl-table .tpl-column .qrcode-item .tpl-panel .image-box{
  height: 200px;
}
...
.tpl-table .tpl-column .qrcode-item .title-desc{
  height: 65px;
...
.qrcode-item.active {
  ...
  background: url(../assets/images/design/h5_selected_icon.png) right bottom
    no-repeat;
}

.tpl-table .tpl-column .tpl-panel .lock-status {
  position: absolute;
  top: 0px;
  right: 0px;
  z-index: 3;
}
.tpl-table .tpl-column .tpl-panel .lock-status .status-bg {
  width: 50px;
  height: 50px;
  background: url(../assets/images/angle-top-right-bg.png) right top no-repeat;
}
.tpl-table .tpl-column .tpl-panel .lock-status .status-bg .img-lock {
  position: relative;
  width: 100%;
  height: 100%;
  background: url(../assets/images/icon-lock.png) no-repeat 30px 6px;
}
</style>