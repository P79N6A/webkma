  ...
  <div class="sms-send-window">
    <el-collapse :accordion="true" @change="handleChange">
      <el-collapse-item name="sms-tpl-panel" class="sms-collapse-panel">
        <div slot="title" style="text-align:left;">短信模板
          <div class="sms-search-panel">
            <div class="form-sms-search" v-if="showSmsTplSearch">总共
              <span class="yellow-light">{{smsTpls.pagination.total}}</span> 个模板
              <com-search placeholder="按短信内容关键字搜索" @searchHandler="smsTplSearchHandler"></com-search>
            </div>
            <span class="black-light">{{collapseText}}</span>
          </div>
        </div>
        <custom-scroll
          :options="{scrollX: true, scrollY: false,scrollbars:true}"
          @scroll-end="loadMoreSMSTpl"
          :refresh="refreshEvt"
        >
          <div class="tpl-body">
            <el-card
              v-if="showSmsTplSearch&& smsTpls.list.length>0"
              v-for="data in smsTpls.list"
              :key="data.smsId"
              shadow="hover"
              class="tpl-item"
    ...
    <el-row class="sms-send-panel">
      <el-col class="sms-customer-panel" :span="13">
        <el-row class="sms-customer-search-panel" style="text-align:left" :gutter="20">
          <el-col :span="13">收信人：
            <com-search placeholder="输入姓名、手机" @searchHandler="smsCustomerSearchHandler"></com-search>
          </el-col>
          <el-col :span="11">
            <div>短信剩余
              <span class="yellow-light" v-text="surplusSms"></span>条
            </div>
          </el-col>
        </el-row>
        <el-transfer
        ...
        <el-row>
          <el-form ref="form" :model="sendSMSForm" label-width="74px" size="mini">
            <el-form-item label="短信签名 :">
              <el-select
                v-if="singNameList.length>0"
                v-model="sendSMSForm.signName"
                size="small"
                placeholder="请选择短信签名"
                style="width:100%;"
                :popper-append-to-body="false"
              >
                <el-option v-for="item in singNameList" :key="item" :label="item" :value="item"></el-option>
              </el-select>
            ...
            <div class="btn-div">
              <el-button
                ...
                type="primary"
                size="small"
                @click.native.stop="sendSMSHandler"
                :disabled="!btnSendSMSenabled || sending"
              >正式发送</el-button>
              <el-button
                style="float:right;margin-right: 13px;"
                type="primary"
                size="small"
                @click.native.stop="saveSMSHandler"
                :disabled="!btnSendSMSenabled || saving"
              >保存</el-button>
            </div>
          </el-form>
        </el-row>
...
export default {
  ...
  computed: {
    ...
    btnSendSMSenabled() {
      ...
      return (
        !!signName &&
        !!templateContent &&
        (isTime === 0 || (isTime === 1 && !!sendTime)) &&
        this.customers.selectedList.length > 0
      );
    }
  },
  methods: {
    ...
    getSMSTplList() {
      ...
      let options = Object.assign(
        { searchStr: self.smsTpls.tplKeywords },
        self.smsTpls.pagination
      );
    ...
    getCustomerList(selectedList) {
      ...
      let options = Object.assign(
        { keyWords: self.customers.keywords },
    ...
    sendSMSHandler() {
      ...
      api.request("sendSMS", body, result => {
        if (result.status == 0) {
          ...
          this.$eventBus.$emit('closeSendSMSWindow');
          self.getSmsResource()
        } else {
          self.$message.error(result.message);
        }
      ...
      if (!!this.selectedSMSTpl && !!this.selectedSMSTpl.smsId) {
        api.request(
          "smsTplQuote",
          { smsId: this.selectedSMSTpl.smsId },
          result => {
            if (result.status == 0) {
    ...
    saveSMSHandler() {
      ...
      api.request("saveSMS", body, result => {
        if (result.status == 0) {
     ...
     getSmsResource() {
      ...
      api.request(
        "getSmsInfo",
        { secretKey: localStorage.businessSecret, type: 1 },
        result => {
  ...
  mounted() {
    ...
    // api.request(
    //   "getSmsInfo",
    //   { secretKey: localStorage.businessSecret, type: 1, pageIndex: 1 },
    //   result => {
    //     if (result.status == 0 && result.data != "用户没有资源") {
    //       self.surplusSms =
    //         parseFloat(result.data.resGiveRemainder) +
...
.sms-send-panel .sms-content-panel .btn-div {
  height: 92px;
  padding-top: 40px;
}
