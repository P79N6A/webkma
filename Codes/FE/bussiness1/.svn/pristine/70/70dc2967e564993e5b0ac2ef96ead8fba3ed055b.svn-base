<template>
  <div style="heigth:auto">
    ...
    <el-table :data="tableData" style="width: 100%" :default-sort="{prop: 'date', order: 'descending'}"
          ...
          <div style="display: flex; margin:10px 0;">
            ...
            <div class="font-12">
             <p class="black-deep" >{{scope.row.w_name?scope.row.w_name:scope.row.u_name}}</p>
                <p class="tipText-color" v-if="scope.row.role!='客户'">{{scope.row.role}}:{{scope.row.e_empl_name}}</p>
            </div>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="exposure_number" label="带来浏览次数" align="center" sortable v-if="isShow==1 || isShow==2"></el-table-column>
        <el-table-column prop="exposure_number" label="带来预约数" align="center" sortable v-if="isShow!=1 || isShow!=2"></el-table-column>
