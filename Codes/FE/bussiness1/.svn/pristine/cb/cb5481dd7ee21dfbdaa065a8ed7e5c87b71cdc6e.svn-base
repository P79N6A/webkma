<template>
  <div style="overflow: hidden;">
    ...
    <div class="container-fluid clear-padding">
      <div class="tpl-table" v-if="navigatorOpts.selectedKey!=='sms'&&navigatorOpts.selectedKey!=='mail'"
      ...
      <div v-if="navigatorOpts.selectedKey==='mail'" style="padding-left: 40px;">
        <mailpage></mailpage>
      </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      navigatorOpts: {
        tabs: [
          { key: "release", title: "我发布的作品" },
          { key: "draft", title: "我的作品素材" },
          { key: "sms", title: "我发送的短信" },
          { key: "mail", title: "我发送的邮件" }
        ],
        selectedKey: ""
      },
      manuscriptItemOpt: {
  ...
  created() {
    ...
    let index = this.navigatorOpts.tabs.findIndex(
      t => t.key === this.navigatorOpts.selectedKey
    );
    if (this.navigatorOpts.selectedKey != 'sms' && this.navigatorOpts.selectedKey != 'mail') {
      this.pageTitleOpt.showSearch = true;
    } else {
      this.pageTitleOpt.showSearch = false;
    }
    this.$set(
      this.navigatorOpts.tabs[index === -1 ? 0 : index],
      "selected",
      true
    );
  },
  ...
  methods: {
    getList(cb) {
      ...
      if (_this.navigatorOpts.selectedKey === "sms" || _this.navigatorOpts.selectedKey === "mail") {
        return;
      }
    ...
    transferTab(tab) {
      ...
      if (tab != 'sms' && tab != 'mail') {
        ...
        self.keyWords = '';
        self.pageTitleOpt.showSearch = true;
      } else {
        self.pageTitleOpt.showSearch = false;
      }
      self.list = [];
      self.getList();
    },
