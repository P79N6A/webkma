      ...
      <div class="content">
        <div class="page-one clearfix" v-show="pageStep==1">
          <div class="pull-left">
            <div class="target module-item">
              ...
              <div class="options clearfix">
                <div
                  class="option"
                  v-for="item in configData.target"
                  :key="item.id"
                  @click="chooseTaget(item)"
                  :class="{ active: item.active, 'pull-left': item.isEven, 'pull-right':!item.isEven}"
                >
                  <i class="el-icon-circle-check" v-if="item.active"></i>
                  <img :src="'/static/images/moments/'+item.logo">
                  {{item.name}}
                </div>
              </div>
            </div>
          ...
          <div class="diagram module-item pull-left">
            ...
            <div class="pic">
              <img src="../../assets/images/moments/diagram.jpg" alt>
            </div>
            <div>
        ...
        <div class="page-two clearfix" v-show="pageStep==2">
          ...
          >
            ...
            <div class="custom module-item">
              ...
              <div class="form-div">
                <el-form-item label="地域：" prop="cityCodes">
                  <selecttree
                    class="w-370"
                    :dataOfLevel="addressList"
                    :dataOfAll="cityAllList"
                    :checkedNodesKeys="data.cityCodes"
                    :cb="setCityCode"
                    type="city"
                  ></selecttree>
                  <div>
                    请选择起投出价相同的城市。若投放不同价格的城市，请新增广告。
                    <a @click="showPriceDialog">查询定价策略</a>
                  </div>
                </el-form-item>
                <el-form-item label="年龄：" prop="ageEnd">
                  <el-select v-model="data.ageBegin" placeholder="请选择" class="w-150" :popper-append-to-body="false">
                    <el-option v-for="item in agesData" :key="item" :value="item">{{item}}</el-option>
                  </el-select>-
                  <el-select v-model="data.ageEnd" placeholder="请选择" class="w-150" :popper-append-to-body="false">
                    <el-option v-for="item in agesData" :key="item" :value="item">{{item}}</el-option>
                  </el-select>
                </el-form-item>
                ...
                <el-form-item label="兴趣：" prop="interestCodes">
                  <selecttree
                    class="w-370"
                    :dataOfLevel="interestList"
                    :dataOfAll="interestAllList"
                    :checkedNodesKeys="data.interestCodes"
                    :cb="setInterestCode"
                  ></selecttree>
                </el-form-item>
              </div>
              <el-dialog
                title="定价策略说明"
                :visible.sync="dialogOptionPrice.visible"
                width="825px"
                center
              ...
              >
                ...
                <div>
                  <img src="../../assets/images/moments/price.jpg">
                </div>
                <div>备注说明</div>
            ...
            <div class="pay module-item">
              ...
              <div class="form-div">
                <el-form-item label="每日预算：" prop="budget">
                  <el-input v-model.number="data.budget" class="w-210"></el-input>
                  <span>日预算范围1000-10000000元/天</span>
                </el-form-item>
                <el-form-item label="出价方式：">
                  <el-radio
                    v-model="data.bidCode"
                    :label="item.code"
                    v-for="item in configData.bid"
                    :key="item.id"
                  >{{item.name}}</el-radio>
                </el-form-item>
                <el-form-item label="出价：" prop="bidPrice">
                  <el-input v-model="data.bidPrice" class="w-210"></el-input>
            ...
            <div class="setting module-item">
              ...
              <div class="form-div">
                <el-form-item label="样式：">
                  <el-radio
                    v-model="data.styleCode"
                    :label="item.code"
                    v-for="item in configData.style"
                    :key="item.id"
                    @change="changeStyle"
                  >{{item.name}}</el-radio>
                  <div>
                    <img
                      v-for="(item,index) in configData.style"
                      :key="item.id"
                      :src="'static/images/moments/style0'+(index+1)+'.jpg'"
                      v-if="data.styleCode==item.code"
                      alt
                    >
                  </div>
                </el-form-item>
                <el-form-item
                  label="外层文案标题："
                  prop="outerCopyTitle"
                  v-if="data.styleCode==configData.style[1].code"
                >
                  <el-input v-model="data.outerCopyTitle"></el-input>
                </el-form-item>
                <el-form-item label="外层文案：" prop="outerContent">
                  <el-input type="textarea" v-model="data.outerContent" :rows="4"></el-input>
                </el-form-item>
                <el-form-item label="外层素材跳转：" prop>
                  <el-radio
                    v-model="data.chickJumpCode"
                    :label="item.code"
                    v-for="(item,index) in configData.chickjump"
                    :key="item.id"
                    v-if="(data.styleCode==configData.style[0].code&&index!=2)||(data.styleCode==configData.style[1].code&&index!=1)"
                  >{{item.name}}</el-radio>
                  <div v-if="data.chickJumpCode==configData.chickjump[0].code">
                ...
                <el-form-item
                  label="外层素材样式："
                  v-if="data.chickJumpCode!=configData.chickjump[0].code"
                >
                  <el-radio
                    v-model="data.outerMaterialCode"
                    :label="item.code"
                    v-for="item in configData.material"
                    :key="item.id"
                  >{{item.name}}</el-radio>
                </el-form-item>
                <el-form-item
                  label="外层图片："
                  prop="outerMaterialPath"
                  v-if="data.outerMaterialCode==configData.material[0].code&&data.chickJumpCode!=configData.chickjump[0].code"
                >
                  <form class="form_upload">
                    <comupload
                      type="outerMaterialPath"
                      class="upload-com"
                      upfileId="upfileId1"
                      size="300KB"
                      uploadTip="jpg/jpeg/png <300KB"
                      :imageUrl="data.outerMaterialPath"
                      fileType="/\.(jpg|jpeg|png|JPG|PNG)$/"
                      :cb="setShareImage"
                    ></comupload>
                  </form>
                  <div class="pic-des">
                    <div>图片尺寸：800*450像素</div>
                    <div>图片格式：大小要求在300KB以内，不支持GIF格式，其中文字占图片篇幅不超过30%</div>
                  </div>
                </el-form-item>
                <el-form-item label="文字链类型：" v-if="data.styleCode==configData.style[0].code">
                  <el-radio
                    v-model="data.textStyleCode"
                    :label="item.code"
                    v-for="item in configData.textstyle"
                    :key="item.id"
                    @change="changeTextStyle(item)"
                    v-if="item.constraint.indexOf(data.targetCode)!=-1"
                  >{{item.name}}</el-radio>
                </el-form-item>
                <el-form-item
                  label="文字链跳转："
                  v-if="data.styleCode==configData.style[0].code&&data.textStyleCode!='TEXT_STYLE_01'"
                >
                  <el-radio
                    v-model="data.textJumpCode"
                    :label="item.code"
                    v-for="item in configData.textjump"
                    :key="item.id"
                    v-if="item.constraint.indexOf(data.textStyleCode)!=-1"
                  >{{item.name}}</el-radio>
                  <div v-if="data.targetCode=='TARGET_05'">微信卡券需要在微信公众号进行设置相应的卡券信息</div>
                </el-form-item>
                <el-form-item label="绑定小程序：" v-if="data.textJumpCode=='TEXT_JUMP_03'&&data.textStyleCode!='TEXT_STYLE_01'">
                  <el-input v-model="data.appOriginalId"></el-input>
                </el-form-item>
                <el-form-item
                  label="分享标题："
                  prop="shareTitle"
                ...
                <el-form-item
                  ...
                  v-if="data.chickJumpCode==configData.chickjump[0].code"
                >
                  <el-input type="textarea" v-model="data.shareDesc" :rows="4"></el-input>
                </el-form-item>
              </div>
              <el-dialog
                title="选择活动推广素材"
        ...
        <div class="page-four clearfix" v-show="pageStep==4">
          <div class="preview module-item">
            ...
            <div class="preview-text">
              <div class="module-item">
                ...
                <div class="text">
                  <el-row>
                    ...
                    <el-col :span="18">
                      <div>{{previewData_targetName}}</div>
                    </el-col>
                  </el-row>
                  <el-row>
                    <el-col :span="6" class="col-title">
                    ...
                    <el-col :span="18">
                      <div>{{previewData_advertName}}</div>
                    </el-col>
                  </el-row>
                  <el-row>
                    <el-col :span="6" class="col-title">
                    ...
                    <el-col :span="18">
                      <div>{{previewData_buywayName}}</div>
                    </el-col>
                  </el-row>
                </div>
              </div>
                ...
                <div class="text">
                  <el-row>
                    ...
                    <el-col :span="18">
                      <div>
                        <span>地域：</span>
                        {{previewData_cityName.substring(0,10)+"..."}}
                        <span>&nbsp;&nbsp;年龄：</span>
                        {{data.ageBegin}}-{{data.ageEnd}}岁
                        <span>&nbsp;&nbsp;性别：</span>
                        {{previewData_sexName}}
                        <span>&nbsp;&nbsp;兴趣：</span>
                        {{previewData_insterestName.substring(0,10)+"..."}}
                      </div>
                    </el-col>
                  </el-row>
                  <el-row>
                    ...
                    <el-col :span="18">
                      <div>{{previewData_timeOnline}} 至 {{previewData_timeOffline}}&nbsp;&nbsp;{{data.timeDayBegin}} - {{data.timeDayEnd}}</div>
                    </el-col>
                  </el-row>
                  <el-row>
                    <el-col :span="6" class="col-title">
              ...
              <div class="module-item">
                ...
                <div class="text">
                  ...
                  <el-row>
                    ...
                    <el-col :span="18">
                      <div>{{previewData_chickJump}}</div>
                    </el-col>
                  </el-row>
                  <el-row v-if="data.chickJumpCode!=configData.chickjump[0].code">
                    <el-col :span="6" class="col-title">
                      <div>外层素材样式</div>
                    </el-col>
                    <el-col :span="18">
                      <div>{{previewData_outerMaterial}}</div>
                    </el-col>
                  </el-row>
                  <el-row v-if="data.chickJumpCode==configData.chickjump[0].code">
                    <el-col :span="6" class="col-title">
                    ...
                    <el-col :span="18">
                      <div>{{previewData_relationName}}</div>
                    </el-col>
                  </el-row>
                  <el-row v-if="data.styleCode==configData.style[1].code">
                    <el-col :span="6" class="col-title">
                      <div>外层文案标题</div>
                    </el-col>
                    <el-col :span="18">
                      <div>{{data.outerCopyTitle}}</div>
                    </el-col>
                  </el-row>
                  <el-row>
                    <el-col :span="6" class="col-title">
                    ...
                    <el-col :span="18">
                      <div>{{data.outerContent}}</div>
                    </el-col>
                  </el-row>
                  <el-row v-if="data.styleCode==configData.style[0].code">
                    <el-col :span="6" class="col-title">
                      <div>文字链类型</div>
                    </el-col>
                    <el-col :span="18">
                      <div>{{previewData_textStyle}}</div>
                    </el-col>
                  </el-row>
                  <el-row v-if="data.styleCode==configData.style[0].code">
                    <el-col :span="6" class="col-title">
              ...
              <div class="des">
                <el-checkbox
                  v-model="isCheckedDes"
                >我方同意并确认遵守广告投放须知，确保广告真实、合法、有效，否则可能被监管机关处罚并面临被处广告3-5倍罚款等后果，并且我方须独立承担全部责任。</el-checkbox>
              </div>
            </div>
          </div>
        </div>
        <div class="btn-div">
          <el-button
            type="primary"
            size="small"
            v-if="pageStep>1&&!isLock"
            @click="setPageStepPre()"
          >上一步</el-button>
          <el-button
            type="primary"
            size="small"
            v-if="pageStep!=4&&!isLock"
            @click="setPageStepNext()"
          >下一步</el-button>
          <el-button
            type="primary"
            size="small"
...
export default {
  ...
  data() {
    ...
    var validateBudget = (rule, value, callback) => {
      if (parseFloat(value) < 1000 || parseFloat(value) > 10000000) {
        callback(new Error("请输入范围1000-10000000元/天预算！"));
      } else if (!/^(?!(0[0-9]{0,}$))[0-9]{1,}[.]{0,}[0-9]{0,2}$/.test(value)) {
        callback(
          new Error(
            "最多2位小数哦！"
          )
    ...
    var validatePrice = (rule, value, callback) => {
      if (
        parseFloat(value) < this.priceRange[0] ||
        parseFloat(value) > this.priceRange[1]
      ) {
        callback(
          new Error(
            "请输入范围" +
    ...
    return {
      ...
      data: {
        ...
        interestCodes: "", //目标人群兴趣
        budget: "", //预算
        bidCode: "", //出价方式
        bidPrice: "", //竞价
        styleCode: "", //内容样式编码
        outerCopyTitle: "", //外层文案标题
        outerContent: "", //外层内容
        outerMaterialPath: "", //分享缩略图
        shareTitle: "", //分享标题
        shareDesc: "", //分享描述
        relationId: "", //活动id
        reason: "", //审核失败原因
        chickJumpCode: "", //点击跳转类型
        outerMaterialCode: "", // 外层素材样式
        textStyleCode: "", //文字链接类型
        textJumpCode: "", //文字链接跳转
        appOriginalId: "" //小程序原始id
      },
      paginationOpt: {
        pageIndex: 1,
        pageSize: 10,
      ...
      addressList: [], // 城市列表
      cityAllList: [], //城市列表所有层级数据
      cityTopKey: "",
      interestList: [], // 兴趣列表
      interestAllList: [], //兴趣列表所有层级数据
      dialogOptions: {
        //活动推广弹框
        visible: false
      },
      ...
      isLock: false, //是否锁定step，用于推广中、暂停编辑
      ...
      rules: {
        ...
        interestCodes: [
          { required: true, message: "请选择定向人群兴趣", trigger: "change" }
        ], //目标人群兴趣
        budget: [
        ...
        outerCopyTitle: [
          { required: true, message: "请输入外层文案标题", trigger: "blur" },
          {
            min: 1,
            max: 10,
            message: "请输入1-10字符外层文案标题",
            trigger: "blur"
          }
        ], //外层文案标题
        outerMaterialPath: [
          { required: true, message: "请选择外层图片", trigger: "change" }
        ], //分享缩略图
        shareTitle: [
      ...
      .then(data => {
        if (!!this.$route.query.id) {
          ...
          if (!!this.$route.query.isLock) {
            this.isLock = true;
            this.pageStep = 2;
            this.isCheckedDes = true;
            if (this.addressList.length == 0 && this.interestList.length == 0) {
  ...
  computed: {
    ...
    previewData_style: function() {
      if (!!this.configData.style) {
        let _item = this.configData.style.find(
    ...
    previewData_chickJump: function() {
      if (!!this.configData.chickjump) {
        let _item = this.configData.chickjump.find(
          item => item.code == this.data.chickJumpCode
        );
        return !!_item ? _item.name : "";
      } else {
        return "";
    ...
    previewData_textJump: function() {
      if (!!this.configData.textjump) {
        let _item = this.configData.textjump.find(
          item => item.code == this.data.textJumpCode
        );
        return !!_item ? _item.name : "";
      } else {
        return "";
    ...
    previewData_textStyle: function() {
      if (!!this.configData.textstyle) {
        let _item = this.configData.textstyle.find(
          item => item.code == this.data.textStyleCode
        );
        return !!_item ? _item.name : "";
      } else {
        return "";
    ...
    previewData_outerMaterial: function() {
      if (!!this.configData.material) {
        let _item = this.configData.material.find(
          item => item.code == this.data.outerMaterialCode
        );
        return !!_item ? _item.name : "";
      } else {
        return "";
    ...
    previewData_targetName: function() {
      if (!!this.configData.target) {
        let _item = this.configData.target.find(
    ...
    previewData_advertName: function() {
      if (!!this.configData.advert) {
        let _item = this.configData.advert.find(
    ...
    previewData_buywayName: function() {
      if (!!this.configData.buyway) {
        let _item = this.configData.buyway.find(
    ...
    previewData_insterestName: function() {
      let _name = "";
      if (!!this.data.interestCodes && this.interestAllList.length > 0) {
        $.each(this.data.interestCodes.split(","), (index, itemCode) => {
          _name =
            _name +
            this.interestAllList.find(item => item.id == itemCode).label +
            ",";
        });
      }
      return _name;
    },
    previewData_cityName: function() {
      let _name = "";
      if (!!this.data.cityCodes && this.cityAllList.length > 0) {
        $.each(this.data.cityCodes.split(","), (index, itemCode) => {
          if (index == 10) {
            return false;
          }
          _name =
            _name +
            this.cityAllList.find(item => item.id == itemCode).label +
            ",";
        });
      }
      return _name;
    },
    previewData_timeOnline: function() {
      return this.data.timeOnline.split(" ")[0];
    },
    previewData_timeOffline: function() {
      return this.data.timeOffline.split(" ")[0];
    },
    cityTopNodeKey: function() {
      let _id = "";
      this.getTopNode(
        this.addressList,
        this.data.cityCodes.split(",")[0],
        id => {
          _id = id;
        }
      );
  ...
  methods: {
    ...
    getMomentsAdvConfigList(cb) {
      ...
      api.request("getMomentsAdvConfigList", {}, result => {
        if (result.status == 0) {
          ...
          _this.data.chickJumpCode = _this.configData.chickjump[0].code;
          _this.data.outerMaterialCode = _this.configData.material[0].code;
          _this.data.textStyleCode = _this.configData.textstyle[0].code;
          _this.data.textJumpCode = _this.configData.textjump[0].code;
          console.log(_this.data.outerMaterialCode);
        } else {
      ...
      //设置年龄范围
      let _agesData = [];
      for (let i = 14; i <= 60; i++) _agesData.push(i);
    ...
    getInterestList(cb) {
      ...
      api.request("getInterestList", {}, result => {
        if (result.status == 0) {
          ...
          _this.getInterestAllList(_this.interestList);
        } else {
          _this.$message.error(result.message);
        }
        !!cb && cb();
    ...
    //获取兴趣列表所有层级数据
    getInterestAllList(items) {
      let _this = this;
      $.each(items, (index, item) => {
        _this.interestAllList.push(item);
        if (!!item.children) {
          _this.getInterestAllList(item.children);
        }
      });
    },
    ...
    getCityList(cb) {
      let _this = this;
      api.request("getCityList", {}, result => {
        if (result.status == 0) {
          _this.addressList = result.data;
          _this.getCityAllList(_this.addressList);
        } else {
          _this.$message.error(result.message);
        }
        !!cb && cb();
    ...
    //获取城市列表所有层级数据
    getCityAllList(items) {
      let _this = this;
      $.each(items, (index, item) => {
        _this.cityAllList.push(item);
        if (!!item.children) {
          _this.getCityAllList(item.children);
        }
      });
    },
    //点击选择推广目标
    chooseTaget(item) {
      ...
      if (!!item.constraint) {
        this.data.textStyleCode = item.constraint;
      } else {
        this.data.textStyleCode = this.configData.textstyle[0].code;
      }
    },
    //点击选择活动推广
    ...
    addMomentsAdv() {
      ...
      if(this.data.chickJumpCode == this.configData.chickjump[0].code){
        if(this.data.relationId == ""){
          this.$message.error("选择原生推广页不能为空！");
          return false;
        }
      }
      if(this.data.chickJumpCode == this.configData.chickjump[1].code){
        if(!this.data.outerMaterialPath){
          this.$message.error("外层素材图片地址不能为空！");
          return false;
        }
      }
      ...
      self.validateData(() => {
        api.request("editMomentsAdv", self.data, result => {
          if (result.status == 0) {
            location.href =
              (location.origin || location.protocol + "//" + location.host) +
              "/#/moments-adv?selectKey=advPromotionManagement";
            if (
              !!/(trident).+rv[:\s]([\w\.]+).+like\sgecko/i.test(
                navigator.userAgent
              ) ||
    ...
    //设置分享缩略图
    setShareImage(data) {
      this.data.outerMaterialPath = data.outerMaterialPath;
    },
    //选择时间
    chooseDate() {
      let _this = this;
    ...
    //获取节点顶级节点Node
    getTopNode(items, id, cb, parentNode) {
      let _this = this;
      $.each(items, (index, item) => {
        if (!parentNode) {
          _this.cityTopKey = item.id;
        }
        if (id == item.id) {
          return cb(_this.cityTopKey);
        }
        if (!!item.children) {
          _this.getTopNode(item.children, id, cb, item);
        }
      });
    },
    setInterestCode(data) {
      this.data.interestCodes = data.keys.join(",");
    },
    setCityCode(data) {
      this.data.cityCodes = data.keys.join(",");
    },
    changeStyle() {
      this.data.chickJumpCode = this.configData.chickjump[0].code;
    },
    changeTextStyle(item) {
      $.each(this.configData.textjump, (index, itemData) => {
        if (itemData.constraint == item.code) {
          this.data.textJumpCode = itemData.code;
          return false;
        }
      });
      if(this.data.textStyleCode=='TEXT_STYLE_01'){
        this.data.textJumpCode =""
      }
    }
  }
};
...
.contain .content .custom .el-dialog__body {
...
.contain .content .setting .form-div .pic-des {
  margin-top: 10px;
  line-height: 23px;
  color: #888;
}
