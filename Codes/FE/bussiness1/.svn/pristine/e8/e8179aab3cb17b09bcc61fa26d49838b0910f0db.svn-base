  ...
  <div class="adv-promotion-management" style="padding-top:20px;">
    <div style="margin-bottom:20px;">
      ...
      <input
        ...
        class="pull-left form-control"
        style="width:17%"
        placeholder="请输入推广计划名称进行搜索"
        v-model="search"
      >
      <span
        style="display:inline-block;width:100px;height:34px;line-height:34px;background:#01bacf;border-radius:0 4px 4px 0;text-align:center;color:#fff;cursor:pointer;"
        @click="searchFn"
      >搜索</span>
    ...
    >
      <el-table-column label="推广计划" width="200">
        <template slot-scope="scope">
          <div>
            <img class="img pull-left" :src="scope.row.cover">
            <span
              v-text="scope.row.activityName"
              style="display:table-cell;height:80px;line-height:1.2158;vertical-align:middle;"
            ></span>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="100"
      ></el-table-column>
      <el-table-column prop="totalShare" label="投放时间" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <div>
      ...
      <el-table-column prop="budget" label="广告预算" align="center"></el-table-column>
      <el-table-column prop="extensionCost" label="花费" align="center"></el-table-column>
      <el-table-column prop="clickCount" label="点击次数" width="70" align="center"></el-table-column>
      <el-table-column prop="exposureCount" label="曝光次数" width="70" align="center"></el-table-column>
      <el-table-column prop="clickRate" label="点击率" align="center" width="70">
        <template slot-scope="scope">{{scope.row.clickRate}}{{!!scope.row.clickRate ? '%': ''}}</template>
      </el-table-column>
      <el-table-column prop="conversionIndex" label="转化指标" align="center" width="70"></el-table-column>
      <el-table-column prop="conversionCost" label="转化成本" align="center"></el-table-column>
      <el-table-column label="操作" align="center" width="80">
        <template slot-scope="scope">
          <i class="btn-plain" v-if="scope.row.state == 1" @click="jumpToCreat(scope.row.mainId)">编辑</i>
          <i
            class="btn-plain"
            v-if="scope.row.state == 2||scope.row.state == 4"
            @click="jumpToCreat(scope.row.mainId,true)"
          >编辑</i>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    chooseDate() {
      let self = this;
      this.timeOnline = !!self.actTime
        ? window.timeFormdate(self.actTime[0])
        : "";
      this.timeOffline = !!self.actTime
        ? window.timeFormdate(self.actTime[1])
        : "";
    },
    //搜索
    searchFn() {
    ...
    jumpToCreat(id, isLock) {
      ...
      if (!!isLock) {
        pArr.push("isLock=" + isLock);
      }
      if (pArr.length > 0) {
        $.each(pArr, (index, item) => {
          if (index == 0) {
            param = param + "?" + item;
          } else {
            param = param + "&" + item;
