                  ...
                  <div class="pull-right info_box">
                      ...
                      <el-form-item prop="name" style="margin-bottom: 8px;">
                        ...
                        >
                          <i
                            v-if="type!=2"
                            slot="suffix"
                            style="font-size: 12px; font-style: normal; line-height: 42px;"
                          >{{nameLength}}/64</i>
                        </el-input>
                      </el-form-item>
                    </div>
                    ...
                    <div v-if="role == '2'">
                      <el-form-item prop="catIds">
                        ...
                        <selecttree
                          class="w-370"
                          style="width: 328px;position: absolute;top: 0;left: 70px;"
                          :isReadonly="true"
                          :dataOfLevel="categoryList"
                          :dataOfAll="categoryAllList"
                          :cb="setCategoryCode"
                        ></selecttree>
                      </el-form-item>
                    </div>
            ...
            <div class="tag">
              <div class="choose_tag">
                <div class="pull-left" style="line-height:35px;">
                  <i class="iconfont icon-plan tipText-color font-22"></i>
                </div>
                <div class="tipText-color" style="display: table-cell;line-height: 25px;">
                  <span v-for="(item,index) in tags.manuscriptLabel" :key="item.labelId">
                    {{item.labelName}}
                    <i class="iconfont icon-close" @click="deleteTag(index)"></i>
                  </span>
                  <span style="background-color:#fff;" v-if="tags.manuscriptLabel.length<3">
                    <el-input
                      v-if="!!isShowNewTag"
                      style="margin-top:2px; color:#63717b;width: 102px;height: 22px;"
                      placeholder="请输入标签"
                      v-model="newTag"
                      maxlength="6"
                      @blur="addTag()"
              ...
              <div class="recommend tipText-color">
                ...
                <div style="display: table-cell;line-height: 25px;">
                  <span
                    v-for="item in tags.recommendLabel"
                    :key="item.labelId"
            ...
            <div class="library" v-if="role != '2'">
              ...
              <div class="box_border clearfix" style="margin:10px;" v-if="active == 0">
                <div class="pull-left" style="text-align: center;">
                  <el-transfer
                    style="text-align: left; display: inline-block; height: 240px;"
                    v-model="employees.selectedList"
                    :titles="['全选', '全选']"
                    :format="{
                                noChecked: '${total}',
              ...
              <!-- 任务奖励（非设计师） -->
              <div
                class="box_border clearfix"
                style="margin-left:0px;"
                v-show="role != '2' && manuscript.manuscriptType != '2' && active == 1"
              >
                <el-row style="padding:20px 0 0 36px;">
                  <div class="ie_left">
                    ...
                    <slideBtn
                      ...
                      active-text="是"
                      inactive-text="否"
                      :prop-value-map="{0:0,1:1}"
                      prop-key="state"
                      @slide-event="rewardState"
                    ...
                    />
                    <i
                      class="iconfont icon-question tipText-color"
                      v-if="active==1"
                      :style="{'top': (manuscript.modify && manuscript.manuscriptStatus == 2 ? '1px': '2px')}"
                      @mouseenter="reward.showRuleInstruction = true"
                      @mouseleave="reward.showRuleInstruction = false"
                    ></i>
                  </div>
                  <div
                  ...
                  >
                    <div
                      ...
                      v-if="manuscript.modify && reward.state != '0'"
                    >注：您当前暂未设置任务奖励</div>
                    <div v-else>
                      <el-table :data="reward.ruleInstructionsData" border style="width: 100%;">
                        <el-table-column prop="name" label="规则项" align="center" width="80"></el-table-column>
                        <el-table-column prop="desc" align="center" label="说明"></el-table-column>
                      </el-table>
                      <div
                        style="padding-left:10px;height: 30px;line-height: 30px;"
                      >注：员工要拿到奖励，必须完成所有的任务目标，完成部分目标奖励不发放</div>
                    </div>
                ...
                >
                  <el-row class="task-reward">
                    <div class="reward-row reward-amount">
                      ...
                      <span
                        class="font-12 red-deep"
                        style="margin: 0 10px 0 5px;"
                        v-if="reward.style == 'part'"
                    ...
                    <div class="reward-row" style="height: 35px;overflow: hidden;position:relative;">
                      <label class="black-deep">奖励人</label>
                      <i class="trai_i"></i>
                      <select
                        class="choose-employee"
                        @change="selectChange($event)"
                        v-model="reward.style"
                      ...
                      <span v-show="reward.style == 'part'">
                        <el-form-item prop="count">
                          <el-input
                            ref="rewardCount"
                            placeholder="请输入数字"
                            v-model.number="reward.rewardEmployeeCount"
                            :disabled="manuscript.modify && manuscript.manuscriptStatus == 2"
                    ...
                    <div class="reward-row target_form">
                      ...
                      <el-form-item prop="targetAccess">
                        <el-input
                          placeholder="请输入数字"
                          v-model="reward.param.targetAccess"
                          :disabled="manuscript.modify && manuscript.manuscriptStatus == 2"
                        ></el-input>
                      </el-form-item>
                      ...
                      <el-form-item prop="targetService">
                        <el-input
                          placeholder="请输入数字"
                          v-model="reward.param.targetService"
                          :disabled="manuscript.modify && manuscript.manuscriptStatus == 2"
                        ></el-input>
                      </el-form-item>
                    </div>
                    <div class="reward-row target_form">
                      ...
                      <el-form-item prop="targetVisitor">
                        <el-input
                          placeholder="请输入数字"
                          v-model="reward.param.targetVisitor"
                          :disabled="manuscript.modify && manuscript.manuscriptStatus == 2"
                        ></el-input>
                      </el-form-item>
                      ...
                      <el-form-item prop="targetDeal">
                        <el-input
                          placeholder="请输入数字"
                          v-model="reward.param.targetDeal"
                          :disabled="manuscript.modify && manuscript.manuscriptStatus == 2"
                        ></el-input>
                      </el-form-item>
                    </div>
...
export default {
  ...
  props: {
    ...
    manuscript: {
      ...
      reward: [],
      manuscriptStatus: "",
      appointMentControlIds: [] //预约controIds
    },
    posterImage: { png: "", jpg: "" },
    previewUrl: "",
    dialogOptions: {
  ...
  computed: {
    ...
    rewordTotal: function() {
      return !this.reward.param.amount
        ? "-"
        : "￥" +
            (
              this.employees.selectedList.length *
              parseFloat(this.reward.param.amount)
            ).toFixed(2);
    },
    partRewordTotal: function() {
      return !this.reward.rewardEmployeeCount || !this.reward.param.amount
        ? "-"
        : "￥" +
            (
              parseInt(this.reward.rewardEmployeeCount) *
              parseFloat(this.reward.param.amount)
            ).toFixed(2);
    }
  },
    ...
    this.$nextTick(() => {
      if (!this.previewUrl && !!this.manuscript.htmlData) {
        setTimeout(() => {
          ...
          this.manuscript.name = this.manuscript.name.replace(/&quot;/gi, '"');
        }, 500);
      }
      if (!!this.previewUrl) {
        this.bindflip();
  ...
  methods: {
      ...
      api
        ...
        .then(function(result) {
          if (result.status == 0) {
            ...
            self.reward.rewardEmployeeCount =
              result.data.count != -1 ? result.data.count : "";
            self.reward.style = result.data.count != -1 ? "part" : "all";
            if (result.data.count != -1)
              $(self.$refs["rewardCount"].$el)
                .find("input")
                .removeClass("error_style")
                .addClass("pass_style");
    ...
    getBussinessBalance() {
      ...
      api
        .request("getAccountDetail", {
          businessId: JSON.parse(localStorage.getItem("userInfo")).businessInfo
            .businessId
        })
        .then(function(result) {
          if (result.status == 0) {
    ...
    bindflip() {
      ...
      $(self.$refs.pageFlipUp)
        .unbind("click")
        .bind("click", function(evt) {
          flipFn(evt, $(this));
          return false;
        });
      $(self.$refs.pageFlipDown)
        .unbind("click")
        .bind("click", function(evt) {
          flipFn(evt, $(this));
          return false;
    ...
    getAssignedEmployeeList() {
      ...
      Promise.all([
        ...
        .then(results => {
          let allEmployee = results[1],
            assignedEmployee = results[0];
    ...
    releaseManuscript(e) {
      ...
      Promise.all([
        ...
        new Promise(function(resolve, reject) {
          if (localStorage.role !== "2" && self.activityData.length == 0)
            return reject(new Error("请选择任务时间！"));
          if (self.type != "2") {
            createPoster(
              {
                cover:
                  self.manuscript.cover ||
                  "https://resource.tuixb.cn/beta/00000000-0000-0000-0000-000000000000/KMA/miniapp/cover-empty.jpg",
              ...
              poster => {
                ...
                self.buildTemplateData(function(result) {
                  options = {
                    ...
                    posterImage: JSON.stringify({
                      png: self.posterImage.png,
                      jpg: self.posterImage.jpg
                    }),
                    posterQrImage: poster,
                    label: self.tags.manuscriptLabel,
                    activeStartDate:
                      self.activityData.length == 0
                        ? ""
                        : timeFormdate(self.activityData[0]),
                    activeEndDate:
                      self.activityData.length == 0
                        ? ""
                        : timeFormdate(self.activityData[1])
                  };
        ...
        new Promise((resolve, reject) => {
          ...
          if (
            localStorage.role == "2" ||
            self.manuscript.manuscriptType == "2" ||
            self.reward.state != "0" ||
            (self.manuscript.modify && self.manuscript.manuscriptStatus == 2)
          ) {
            resolve(null);
          } else {
            // 校验
            self.$refs["rewardparam"].validate(valid => {
              if (valid) {
                ...
                if (self.verifyEmployeeCount()) {
                  if (
                    parseFloat(self.rewordTotal.substring(1)) >
                    parseFloat(self.reward.balance)
                  ) {
                ...
                reject(
                  new Error("发布失败（请检查任务目标是否开启，并未设置）!")
                );
              }
            });
          }
        ...
        .then(function(results) {
          ...
          return new Promise((resolve, reject) => {
            ...
            if(
              self.reward.state == 0 &&
              self.reward.param.targetAccess == 0 && self.reward.param.targetService == 0 &&
              self.reward.param.targetVisitor ==0 && self.reward.param.targetDeal == 0
            ){
              Kdo.utils.messenger.error("目标设置不能全部为0！");
              self.releaseStatus = false;
              return;
            }
        ...
        .then(function(err, cb) {
          return new Promise((resolve, reject) => {
            switch (true) {
              ...
              case self.manuscript.modify &&
                self.manuscript.manuscriptStatus == 2: //修改进行中的任务
              case !self.manuscript.modify && self.reward.state != "0": //创建任务且关闭任务奖励
              case self.manuscript.modify &&
                self.manuscript.reward.length == 0 &&
                self.reward.state != "0": //编辑 + 没有奖励 + 奖励开关关闭
                Kdo.utils.messenger.success(
                  apiName == "releaseWechatAticle"
                    ? "发布文章成功！"
                    : "发布活动成功！"
                );
              ...
              case self.manuscript.modify &&
                self.reward.state != "0" &&
                self.manuscript.reward.length > 0: //编辑 + 已有奖励 + 奖励开关关闭，走删除奖励流程
                // 删除任务奖励
                api.request(
                  "deleteAward",
                  { id: self.manuscript.reward[0].id },
                  result => {
                    if (result.status == 0) {
                      Kdo.utils.messenger.success(
                        apiName == "releaseWechatAticle"
              ...
              case self.manuscript.modify &&
                self.reward.state == "0" &&
                self.manuscript.reward.length > 0: //编辑 + 已有奖励 + 奖励开关关闭，走修改奖励流程
                // 参数里面增加任务奖励id
                self.reward.param.mainId = self.manuscript.reward[0].id;
              default:
                // 设置任务奖励
                let option = Object.assign(self.reward.param, {
                  relationId: data.id,
                  employeeCount: self.employees.selectedList.length,
                  endTime: timeFormdate(self.activityData[1])
                });
    ...
    buildTemplateData(callback) {
      ...
      //添加预约的插件类型
      _.each(self.manuscript.appointMentControlIds, item => {
        params.activityControls.push({
          id: item,
          type: 6
        });
      });
    ...
    verifyEmployeeCount($event) {
      let self = this;
      if (
        self.reward.style == "part" &&
        (!/^[1-9]\d*$/.test(self.reward.rewardEmployeeCount) ||
          parseInt(self.reward.rewardEmployeeCount) >
            self.employees.selectedList.length)
      ) {
        $(self.$refs["rewardCount"].$el)
          .find("input")
          .removeClass("pass_style")
          .addClass("error_style");
        return false;
      } else {
        $(self.$refs["rewardCount"].$el)
          .find("input")
...
.preview-viewport .feature-bar .down:before {
  content: "";
  border-bottom: 10px solid #9eabb8;
