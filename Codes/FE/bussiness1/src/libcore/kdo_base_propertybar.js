    ...
    */
    ...
    var _init = function () {
        ...
        _$propertybar.off("mouseenter.propertybar").on("mouseenter.propertybar", function (event) {
            ...
            //恢复原本的事件
            setTimeout(function () { Kdo.events.on("mouse"); });
        });
        ...
        if (!isEqual || _currentBox.focusLevel != "single") {
            ...
            switch (_currentBox.focusLevel) {
                ...
                case "group":
                    ...
                    Kdo.animation.destroy();
                    eventBus.$emit('componentItemGet', "")//mars 2.13 组合隐藏设置
                    break;
                case "temp":
                    _controlConfig = {
    ...
    var _fontEditorOptions = {
        ...
        fontFamily: {
            "Helvetica": { val: "helvetica" }, // Helvetica
            "微软雅黑": { val: "msyh" }, // 'Microsoft YaHei'
            "黑体": { val: "simhei" }, // SimHei
            "宋体": { val: "simsun" }, // SimSun
            "楷体": { val: "simkai" }, // KaiTi
            "幼圆": { val: "simyou" }, // YouYuan
            // "手书": { val: "shoushu" }, // 字体源文件有问题
            "颜体": { val: "i-ngaan" },
            "庞门正道": { val: "pmzd" },
            // "锐字真言": { val: "rzzyt" }, // 字体源文件有问题
            "沐瑶软笔手写": { val: "muyao-softbrush" },
            "智勇手书英文": { val: "zhiyong" },
            "站酷意大利英文": { val: "zcool-addict-italic" }
        }
    }
