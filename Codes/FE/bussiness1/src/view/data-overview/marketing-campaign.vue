    ...
    <div>
       ...
       <div class="content-body">
         <el-table :data="marketingCampaign" @sort-change="sortChangeHandler" tooltip-effect="dark" style="width: 100%" row-key="id" class="table" header-row-class-name="table-header" header-cell-class-name="table-header">
                  <el-table-column label="活动名称"  width="200" prop="name" show-overflow-tooltip ></el-table-column>
                  <!-- <el-table-column label="活动分类"  width="200" prop="_category" show-overflow-tooltip ></el-table-column> -->
                  <el-table-column prop="_cats" label="活动分类" width="200">
                    <template slot-scope="scope">
                      <el-tag :type="'primary'" size="mini" style="margin:3px 0 0 3px;" disable-transitions v-for="item in scope.row._cats" :key="item.catId">{{item.name}}</el-tag>
                      <!-- <el-tag :type="'primary'" disable-transitions>{{scope.row._cats}}</el-tag> -->
                    </template>
                  </el-table-column>
                  <el-table-column label="发布时间"   width="150" prop="createTime" show-overflow-tooltip sortable="custom" align="center" class-name="number_color"></el-table-column>
                  <el-table-column label="累计推广分享数"  width="130" prop="totalShare" show-overflow-tooltip sortable="custom" align="center" class-name="number_color"></el-table-column>
                  <el-table-column label="累计推广访客数"  width="130" prop="totalVisitor" show-overflow-tooltip sortable="custom" align='center' class-name="number_color"></el-table-column>
                  <el-table-column label="累计推广浏览数"  width="130" prop="totalAccess" show-overflow-tooltip sortable="custom" align='center' class-name="number_color"></el-table-column>
    ...
    getManuscriptListOfRelease(cb){//获取已发布稿件列表
      ...
      var obj={
       ...
       api.request("getManuscriptListOfRelease", obj, result => {
          if(result.status==0){
            self.marketingCampaign = result.data.list.map(item => {
              item._cats = JSON.parse(item.cats || "[]");
              // item._category = item._cats.map(cat=>{return cat.name}).join(", ") || "无";
              return item;
            });
