      ...
      <div id="content-resize-box">
        <div style="overflow:hidden;">
          <div id="container">
            ...
            <div class="appointment-bar" :class="{'active' : !!appointSelected}" @click="onParentHandler" v-if="appointment">预约</div>
            <!-- <div class="sidebar" style="background:transparent;"></div> -->
          </div>
        </div>
      </div>
      <div class="phone-footer">
        <div id="resizeHandle" v-if="$route.query.manuscriptType!=2">
          <!--活动类型为海报，将不展示互动和小游戏 -->
...
export default {
  ...
  methods: {
    ...
    //触发预约设置
    onParentHandler() {
      this.appointSelected = true;
      eventBus.$emit('componentItemGet', 'appointmentSetting');
    }
  },
  mounted() {
    var self = this;
    ...
    //页面级resizable功能
    if (this.$route.query.manuscriptType != 2)//活动类型为海报，将不展示互动和小游戏 
    {
    ...
    eventBus.$on('appointReflesh', function() {
      self.appointSelected = false;
    })
    //------------------------画布功能、事件注册 END------------------------
...
.appointment-bar.active {
  color: rgba(0,186,208,1);
}
</style>
