    ...
    <div class="menu-body">
      ...
      <div class="background-color-div" v-show="selected.category=='background'" style="flex-basis: 60px;flex-shrink: 0;">
        ...
        <div class="pull-left color-list">
          <div class="pull-left color-item" v-for="item in backgroundColor" :key="item.index" :style="{backgroundColor:item}"
            @click="setBackgroundColor(item)"></div>
        </div>
      </div>
      ...
      <div class="btn-group" v-if="selected.role == 'own'" style="flex-shrink:0;">
        ...
        <button class="pull-left batch-delete" v-show="selected.editorMaterial" @click="deleteSelectedMaterial()"
          :disabled="!selectedMaterial">删除</button>
        ...
        <form enctype="multipart/form-data" v-show="!selected.editorMaterial" style="float:left;position:relative;width: 100px;overflow: hidden;">
          <input id="myImage-upload" type="file" name="fileName" multiple="multiple" accept="image/png, image/jpeg, image/gif"
            class="input-upload" @change="uploadImg($event)">
      ...
      <custom-scroll :options="{scrollX: false, scrollY: true, scrollbars: true, preventDefault: false}" @scroll-end="loadNextData()"
        :refresh="'refreshScroll'" style="flex-grow: 1;">
        <div style="position: relative; float: left; width: 100%;">
          <!--文字文本-->
          <div v-show="selected.category=='controls_text'">
            <div class="Text" v-for="item in defaultTextList" :key="item.index" data-type="control" :data-key="item.key"
              data-create-type="drag">
              <p>{{item.name}}</p>
            </div>
          </div>
          <!--背景设置-->
          ...
          <div v-if="selected.category!='background'">
            <div class="material" v-for="item in dataList" :key="item.index" :data-type="item.pluginType ||'control'"
              :data-key="item.key" data-create-type="drag" @click="selectMaterial(item)">
              <i class="icon-selected" :class="item.editor ? 'el-icon-circle-check' : 'el-icon-circle-check-outline'"
                v-show="selected.editorMaterial"></i>
              <img :src="item.coverPicture" :title="item.name">
              <div class="mask" v-if="item.selected==false"></div>
              <i class="iconfont icon-lock" v-if="item.selected==false"></i>
              <div v-if="selected.category=='plugins'||selected.category=='qrcode'||selected.category=='games'" class="text-center"
                style="height:30px;line-height:30px;font-size:14px;color:#fff;">{{item.name}}</div>
            </div>
          </div>
          <!--点击模块（背景）列表-->
          <div v-if="selected.category=='background'">
            <div class="material" v-for="item in dataList" @click="selected.editorMaterial ? selectMaterial(item) : setBackground(item)"
              :key="item.index" data-type="background" :data-key="item.key">
              <i class="icon-selected" :class="item.editor ? 'el-icon-circle-check' : 'el-icon-circle-check-outline'"
                v-show="selected.editorMaterial"></i>
              <img :src="item.coverPicture" :title="item.name">
            </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      selected: {
        ...
        pageBackground: (Kdo.data.config.page.get()["style"] || {})[
          "background-color"
        ], //页面设置背景色值
  ...
  mounted() {
    ...
    self.getInitData();
    self.getTagList();
    self.initColorInput(self.selected.pageBackground);

    eventBus.$on("left-menu.refresh", function () {
      self.selected.pageBackground = Kdo.data.config.page.get()["style"]["background-color"];
      $(".left-menu .backgroundColor").spectrum("set", self.selected.pageBackground);
    });
  },
  computed: {
    selectedMaterial() {
    ...
    categoryList() {
      ...
      } else {//活动类型为海报，将不展示互动和小游戏
        return materialCategory.filter(item => {return item.key != "plugins" && item.key != "games" })
      }
    }
  },
  updated() { },
  methods: {
    ...
    getInitData: function () {
      ...
      var callback = function () {
        self.droppable();
        self.refreshScroll(
          Object.assign({ preventDefault: self.selected.category == "background" })
        );
      };
      ...
      switch (self.selected.category) {
        // 文字
        case "controls_text":
          var _text_json = Kdo.menus
            .getLeftMenuJson("controls")
            .find(n => n.key == "controls_text").childs;
          self.defaultTextList = _.filter(_text_json, n => n.struct == "text");
          break;
        // 形状
        case "controls_shape":
          let _contols_rectangle = Kdo.menus
            .getLeftMenuJson("controls")
            .find(n => n.key == "controls_rectangle").childs;
          // 格式化常规样式在菜单中显示
        ...
        // 二维码
        case "qrcode":
          let _pluginqrcode_app = Kdo.menus
            .getLeftMenuJson("plugin")
            .find(n => n.key == "plugin_app");
        ...
        // 活动插件
        case "plugins":
          // let _plugin = _.filter(
          //   Kdo.menus.getLeftMenuJson("plugin"),
          //   n => n.key != "plugin_dynamic"
          // );
          let _plugin = Kdo.menus.getLeftMenuJson("plugin")
          // debugger
          let _userPlugins =
            self.$store.getters.getUserInfo.businessInfo.plugins;
          let _pluginChilds = [];
          $.each(_plugin, (i, item) => {
          ...
          $.each(_pluginChilds, (i, item) => {
            if (item.key != "plugin_app_qrcode") {
              item.coverPicture = require("../../" + item.coverPicture);
              if (localStorage.role == 1 && !item.selected) {
                // if(item.key=="plugin_goods"){
                //   return;
                // }
                if (!!_userPlugins && _userPlugins.indexOf(item.key) != -1) {
                  item.selected = true;
                } else {
                  item.selected = false;
                }
        ...
        case "games":
          ...
          let _gameChilds = [];
          let _userGames = self.$store.getters.getUserInfo.businessInfo.games;
          ...
          $.each(_gameChilds, (i, item) => {
            ...
            if (localStorage.role == 1) {
              if (!!_userGames && _userGames.indexOf(item.key) != -1) {
                item.selected = true;
              } else {
                item.selected = false;
              }
    ...
    getMaterialList(category, cb) {
      ...
      api.request(
        ...
        result => {
          if (result.status != 0)
            return Kdo.utils.messenger.error("获取素材异常");
          //判断是否还有下一页
          ...
          !!cb && cb(list.map(item => {
            return Object.assign({ editor: false }, item);
          }));
        }
      );
    },
    ...
    initColorInput(originalColor) {
      ...
      var _setColor = function (color) {
        ...
        Kdo.data.config.page.set({
          style: { "background-color": !!color ? color.toHexString() : "" }
        });
      };
    ...
    setBackgroundColor(item) {
      let self = this;
      Kdo.data.config.page.set({
        style: { "background-color": !!item ? item : "" }
      });
      self.selected.pageBackground = !!item ? item : "";
      $(".left-menu .backgroundColor").spectrum("set", self.selected.pageBackground);
    },
    //上传图片
    uploadImg(event) {
    ...
    deleteSelectedMaterial() {
      var self = this,
        selectedList = self.dataList.filter(item => {
          return item.editor;
        });
      if (selectedList.length > 0) {
        self
          .$confirm("确定要删除已选中的素材吗？删除后不可恢复哟", "提示", {
            center: true,
            confirmButtonText: "确定",
            cancelButtonText: "取消"
          })
      ...
      setTimeout(function () {
        ...
        $element.find("[data-create-type='drag']").draggable({
          ...
          start: function (event, ui) {
            ...
            dataItem = Kdo.menus.getMenuInfo(
              $(ui.helper.context).data("key"),
              "mobile" /* $scope.tpl_status.webType*/
            );
          ...
          stop: function (event, ui) {
            ...
            // $(".draggable-item .icon-lock").css({ visibility: "visible" });
            dataItem = null;
          },
          drag: function (event, ui) {
            let $dom = $(ui.helper);
            if ($("." + moveDomClassName).length > 0) {
              $("." + moveDomClassName).css({
                top: ui.offset.top,
                left: ui.offset.left
              });
            } else {
              var newDom = $(ui.helper).clone();
              newDom.find(".icon-lock").remove();
            ...
            $element
              .find(".ui-draggable.ui-draggable-handle.ui-draggable-dragging")
              .html("");
          }
        });
      });
