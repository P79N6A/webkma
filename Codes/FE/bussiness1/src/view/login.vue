...
<template>
  <div id="app-login" class="app-login" v-if="!beforload">
    <!--<comHeader/>-->
    <div class="header">
        ...
        <div class="app-login-box-container">
          <form>
            <div class="form-item">
              ...
              <input type="text" class="txt-box" :class="{'txt-placeholder':this.username.length == 0}" v-model="username"
                id="username" name="login" placeholder="用户名">
              <!--</div>-->
            ...
            <div class="form-item">
              ...
              <input type="password" class="txt-box" :class="{'txt-placeholder':this.password.length == 0}" v-model="password"
...
export default {
  ...
  created() {
    let _this = this;
    if (!!_this.$route.query.session_id) {
      api.request("getLogonInfo", { session_id: _this.$route.query.session_id }).then(result => {
        if (result.status == 0) {
          _this.setLoginUserInfo(result)
        } else {
  ...
  methods: {
    ...
    setLoginUserInfo(result) {
      ...
      if(!!this.$route.query.url){
        this.$router.push({ path: decodeURIComponent(_this.$route.query.url) });
      } else {
        switch (sessionInfo.role) {
          case 1:
            ...
            break;
          case 2:
            this.$router.push({ path: "/design-workbench" });
            break;
        }
      }
    },
...
.app-login
  ...
  .txt-placeholder::-webkit-input-placeholder {
  color: #c9d4df;
}
...
.app-login
  .app-login-box-form
  .app-login-box-container
  .txt-placeholder::-moz-placeholder {
  color: #c9d4df;
}
...
.app-login
  .app-login-box-form
  .app-login-box-container
  .txt-placeholder:-ms-input-placeholder {
  color: #c9d4df;
}
