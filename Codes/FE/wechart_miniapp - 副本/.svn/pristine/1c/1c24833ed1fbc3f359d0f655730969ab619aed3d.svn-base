...
module.exports = {
  ...
  getFormContent: function(options, callback) {
    let session_id = getApp().globalData.userInfo.session_id
    request.get({
      url: `${config.apiHost}/activity/plugin/marketing/form/content/list?relationId=${options.id}&controlId=${options.controlId}&pageSize=${options.pageSize}&pageIndex=${options.pageIndex}&session_id=${session_id}`,
      success: (res) => {
        callback(null, res.data);
      },
      fail: (res) => {
