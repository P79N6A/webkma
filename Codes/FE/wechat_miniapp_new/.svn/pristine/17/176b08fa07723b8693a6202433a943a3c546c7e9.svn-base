  ...
  onLoad: function(options) {
    ...
    new Promise((resolve, reject) => { 
      ...
      activityService.getforwardKey(option, (err, res) => {//获取转发者forwardkey
        if (!!res && res.status == 0) {
          self.data.analysisObj.myforwardkey = res.data.forwardKey;
          self.data.analysisObj.rootUserId = res.data.rootUserId;
