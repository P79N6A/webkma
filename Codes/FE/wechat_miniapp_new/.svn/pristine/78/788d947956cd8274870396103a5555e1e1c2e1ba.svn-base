  ...
  buildImg: function() {
    ...
    wx.createSelectorQuery().select('.cover_box').fields({
      ...
      Promise.all([
        new Promise((resolve, reject) => {
          // cover image info
          // let params = [`c_1`, `x_0`, `y_0` , `h_${Math.round(box.height - 110)}`];
          let params = [`s_2`, `w_${Math.round(box.width)}`, `h_${Math.round(box.height - 110)}`];
          wx.getImageInfo({
        ...
        new Promise((resolve, reject) => {
          ...
          activityService.creatQrcode(JSON.stringify({
            ...
            "src": `${_url}?rootUserId=${self.data.userId}&forwardKey=${self.data.myforwardKey}`
          }), function (err, res) {
            if (res.status == 0) {
        ...
        console.log(qrcodeInfo.source.path)
        ...
        ctx.drawImage(coverInfo.source.path,
          0, 0, coverInfo.size.width, coverInfo.size.height, coverInfo.size.width, coverInfo.size.height
        );
  ...
  buildPostImg: function () {
    ...
    wx.createSelectorQuery().select('.cover_box').fields({
      ...
      Promise.all([
        ...
        if (_busName_width - 10 < ctx.measureText(nickname).width + ctx.measureText(phone).width) {
          //一行显示不下时
          //绘制职位
